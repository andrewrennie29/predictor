class MatchesController < ApplicationController
	...
	def importdata
		...
		if season = '1415'
			...
			newdata = DataUpdates.where('newdata = true')
			
			leagues = []
			
			newdata.each do |n|
			
				divs = n.divisions.split(',')
				
				divs.each do |d|
				
					getresultsdata(baseURL + season + "/" + d + ".csv", season)
				
				end
			
				DataUpdates.update(n.id, :newdata => false)
			...
			if match.count==0
				...
				if row["Date"] < Date.today
					...
					if !forecast.nil?
						...
						if forecast.FTHG == match.FTHG && forecast.FTAG == match.FTAG
				
							forecastscore = true
						
							betstake = forecast.bet_stake
							betodds = forecast.bet_odds
						
							unless betstake.nil?
						
								betreturn = betstake.to_f * betodds.to_f
	...
	def getfixturesdata(url)
		...
		updateURL = 'http://www.football-data.co.uk/matches.php'
		
		page_content = open(updateURL).read
		page_updated = page_content.scan(/Latest fixtures uploaded: (.*) UK time./i).first
		
		lastupdated = Date.strptime(page_updated[0], '%d/%m/%y')
		
		du = DataUpdates.where('url = ? ', updateURL).first
		
		if du.nil?
		
			DataUpdates.create(:url => updateURL, :lastupdated => lastupdated)
		
		end
		
		unless lastupdated == du
			...
			CSV.parse(csv_text, :headers => true).each do |row|
				...
				if fixture.count==0 && match.count == 0
					...
					fixture = Fixtures.create!(row.to_hash.select { |k, v| fields_to_insert.include?(k) })
				
					homeform = Matches.where('HomeTeam = ?', fixture.HomeTeam).order(date: :desc).limit(20)
				
					awayform = Matches.where('AwayTeam = ?', fixture.AwayTeam).order(date: :desc).limit(20)
				
					Fixtures.update(fixture.id,
													:phform => homeform.where('correctscore = true and Date >= ?', homeform.minimum(:Date)).count,
													:paform => awayform.where('correctscore = true and Date >= ?', awayform.minimum(:Date)).count,
													:phgames => homeform.count,
													:pagames => awayform.count)
				end
					
			end
			
			DataUpdates.update(du.id, :lastupdated => Date.today)	
	...
	def newdata
		...
		countries = ['england','scotland','germany','italy','spain','france','netherlands']
		
		countries.each do |c|
		
			updateURL = 'http://www.football-data.co.uk/' + c + 'm.php'
			
			page_content = open(updateURL).read
			page_updated = page_content.scan(/Last updated: (.*)I/i).first
		
			lastupdated = Date.strptime(page_updated[0].gsub(/\D/, ''), '%d%m%y')
		
			du = DataUpdates.where('url = ? ', updateURL).first
		
			if du.nil?
		
				du = DataUpdates.create(:url => updateURL, :lastupdated => lastupdated)
			
			end
			
			if lastupdated > du.lastupdated
			
				DataUpdates.update(du.id, :newdata => true)
													
			else
			
				DataUpdates.update(du.id, :newdata => false)
